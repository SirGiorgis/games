...
func _init() -> void:
	...
	_panel(field, 176, 396, 404, 292)
	_panel(field, 700, 396, 404, 292)
	...
	_stamp_fighter(field, fogas, "ultimate", 8, 378, 608, false)
	var gas: Image = Pix.gas()
	gas.resize(gas.get_width() * 3, gas.get_height() * 3, Image.INTERPOLATE_NEAREST)
	field.blend_rect(gas, Rect2i(0, 0, gas.get_width(), gas.get_height()), Vector2i(318, 560))
	_label(field, "FOGAS", Color(0.92, 0.86, 0.70), 378, 620)
	_label(field, fogas.ultimate_name.to_upper(), Color(0.98, 0.86, 0.32), 378, 644)
	_wrap(field, "Stun blast, then poison cloud.", Color(0.88, 0.86, 0.80), 196, 668, 32)

	_stamp_fighter(field, giannis, "ultimate", 10, 902, 608, false)
	_label(field, "GIANNIS", Color(0.35, 0.82, 0.92), 902, 620)
	_label(field, giannis.ultimate_name.to_upper(), Color(0.98, 0.86, 0.32), 902, 644)
	_wrap(field, "Rips the hoodie. Pushes all. ATK x1.42.", Color(0.88, 0.86, 0.80), 720, 668, 32)
