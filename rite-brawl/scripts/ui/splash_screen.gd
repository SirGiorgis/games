...
func _ready() -> void:
	...
	_chris.scale = Vector2(2.35, 2.35)
	_giorgis.scale = Vector2(-2.35, 2.35)
	...
	tw.tween_property(_title, "scale", Vector2.ONE, 0.32)
	tw.parallel().tween_property(_title, "modulate", Color.WHITE, 0.32)
	tw.parallel().tween_property(_chris, "scale", Vector2(3.6, 3.6), 0.34)
	tw.parallel().tween_property(_giorgis, "scale", Vector2(-3.6, 3.6), 0.34)
...
func _process(delta: float) -> void:
	...
	if _chris and not _walk_c.is_empty():
		...
		if _t > 0.36:
			...
			_chris.scale = Vector2(3.6 * b, 3.6 * b)
	if _giorgis and not _walk_g.is_empty():
		...
		if _t > 0.36:
			var b2: float = 1.0 + sin(_t * 2.2 + 1.0) * 0.012
			_giorgis.scale = Vector2(-3.6 * b2, 3.6 * b2)
	if _press:
