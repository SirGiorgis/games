...
func _splash() -> void:
	...
	var chris := _def("chris_xrisakis")
	var giorgis := _def("hoodrich_stacks")
	_stamp_fighter(field, chris, "idle", 0, 430, 520, false)
	_stamp_fighter(field, giorgis, "idle", 0, 850, 520, true)
	var title: Image = PixelFont.make("RITE BRAWL", Color(0.95, 0.18, 0.28), 5).get_image()
	_blit(field, title, 640 - title.get_width() / 2, 88)
	var sub: Image = PixelFont.make("CHOOSE YOUR RITE", Color(0.92, 0.74, 0.32), 2).get_image()
